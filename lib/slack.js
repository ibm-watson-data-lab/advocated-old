...
var formatPost = function(obj, callback) {
  ...
  if (obj.collection == "session") {
    ...
    str += "Hey! *" + obj.user.display_name + "* just `/advocated` with the press @ *" + obj.outlet + "*";
    str += ' about *' + obj.title + '*';
    if (obj.url) {
      str += ' --> ' + obj.url;
    }
...
var post = function(str, callback) {
  ...
  var webhookurls = webhook.split(',');

  var tasks = [];

  for (var i in webhookurls) {
    var webhookurl = webhookurls[i];
    (function(u) {
